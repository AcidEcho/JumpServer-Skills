...
`jumpserver-skills` is a natural-language operations skill repository for JumpServer V4.10. It supports object lookup, permission readback, audit investigation, governance inspection, access analysis, current-identity KoKo SSH and multi-database connections/execution, template-based usage reports, and controlled object creation. Users do not need to manually compose script commands. Persistent business writes remain limited to allowlisted create entrypoints; Access may create only a short-lived, non-reusable connection token after explicit confirmation, and create output redacts secret fields automatically.
...
| User effective access | "Which assets/nodes/accounts/protocols can this user access?" | Query: `subskills/query/scripts/jms_access.py user-assets/user-nodes/user-asset-access` | Returns effective access with current-organization permission-rule evidence and never creates a connection token |
...
| Current-identity SSH command execution | "Run hostname on this host through JumpServer" | Access: `subskills/access/scripts/jms_ssh_exec.py ssh-exec` | Authenticates once after confirmation and reuses one SSH client for all commands; never invokes a local shell or returns the one-time password |
| PostgreSQL/Redis/MongoDB/SQL Server/Oracle/ClickHouse | "Run SELECT version() on PostgreSQL" or "Redis PING" | Access: `jms_connect.py connect-info` / `jms_db_exec.py db-exec` | Requires the exact permitted asset protocol and `is_reusable=false`; PostgreSQL, Redis, MongoDB, SQL Server, and Oracle require `db_client`; ClickHouse requires `web_cli`; every database uses the Smart Endpoint KoKo SSH `host/ssh_port` |
...
| Current-identity SSH command execution | "Run `hostname` and `id -u` through JumpServer" | Access: `subskills/access/scripts/jms_ssh_exec.py ssh-exec --confirm` |
| Multi-protocol database execution | "Run a command on PostgreSQL/Redis/MongoDB/SQL Server/Oracle/ClickHouse" | Access: `subskills/access/scripts/jms_db_exec.py db-exec --confirm` |
...
| "Run a remote SSH command through JumpServer" | Current API identity `ssh-exec`; one authentication and one reused client, with confirmation required before token creation |
| "Connect to PostgreSQL/Redis/MongoDB/serversql/Oracle/ClickHouse" | Current API identity `jms_connect.py` or `jms_db_exec.py`; all database transports use the KoKo SSH Smart Endpoint |
...
| How Access connects to MySQL/MariaDB and runs SQL | [database-connection.md](./subskills/access/references/database-connection.md) | `db-query`, protocol selection, `db_client` token, dynamic SSH `host/ssh_port` from Smart Endpoint, JumpServer SQL policy, and token lifecycle |
| How Access connects to and executes on SSH and multiple databases | [koko-execution.md](./subskills/access/references/koko-execution.md) | `jms_connect.py`, `ssh-exec`, `db-exec`, aliases, protocol framing, Smart Endpoint transport, and credential lifecycle |

...
_GLOBAL_USER_MESSAGE = "创建资产授权规则时，目标组织不能使用全局组织 ID。"


def _raise_global_target_org_error(org_id: str) -> None:
    raise_create_global_org_error(
        org_id,
        resource_name="资产授权规则创建",
...
def _env_org_context() -> dict[str, Any]:
    return create_env_org_context(
        resource_name="资产授权规则创建",
        missing_user_message="未传 `--org-id/--org-name`，且 `.env JMS_ORG_ID` 为空，无法确定资产授权规则创建组织。",

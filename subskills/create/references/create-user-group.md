...
- 无 `--confirm` 只预览 payload，不调用 client，不做在线查重或用户 ID 存在性校验。
- 有 `--confirm` 时，先查重用户组名称，再逐个校验用户 ID 是否存在，最后 POST 创建。
- 用户组名称重复时返回 `reason_code=user_group_already_exists` 与 `duplicate_user_groups`，每条包含 `id`、`name`、`comment`。

...
"""Governance and inspection domain functions for JumpServer inspect subskill."""
...
def security_policy_check(filters: dict[str, Any]) -> dict[str, Any]:
    org_id = _org_id_from_filters(filters)
    rows = _settings_slice(("security", "password", "login", "lock", "block", "ip"), org_id=org_id)
    blocked_ips = create_client(org_id=org_id).get("/api/v1/settings/security/block-ip/")
...
def audit_retention_check(filters: dict[str, Any]) -> dict[str, Any]:
    rows = _settings_slice(("audit", "log", "replay", "command_storage", "retention", "keep"), org_id=_org_id_from_filters(filters))
...
def mfa_config_check(filters: dict[str, Any]) -> dict[str, Any]:
    rows = _settings_slice(("mfa", "otp", "totp", "duo"), org_id=_org_id_from_filters(filters))
    return _with_org_context({"summary": {"matched_keys": len(rows)}, "records": rows})

def auth_source_config_check(filters: dict[str, Any]) -> dict[str, Any]:
    rows = _settings_slice(("ldap", "oidc", "oauth", "saml", "auth", "cas"), org_id=_org_id_from_filters(filters))
    return _with_org_context({"summary": {"matched_keys": len(rows)}, "records": rows})

def notification_config_check(filters: dict[str, Any]) -> dict[str, Any]:
    org_id = _org_id_from_filters(filters)
    rows = _settings_slice(("mail", "sms", "slack", "dingtalk", "feishu", "lark", "wecom", "notify"), org_id=org_id)
    backends = create_client(org_id=org_id).get("/api/v1/notifications/backends/")
...
def _extract_component_id(item: dict[str, Any]) -> str:
    return _extract_identifier(item, "id", "terminal_id", "terminal.id", "service_id")
...
def component_load_overview(filters: dict[str, Any]) -> dict[str, Any]:
    payload = _normalize_time_filters(dict(filters), default_days=1)
    ...
    for item in terminals:
        ...
        has_metric = any(
            ...
            for key in ("cpu_usage_percent", "memory_usage_percent", "disk_usage_percent", "load_value")
        )
        if has_metric:
        ...
        records.append(
            {
                "component_id": _extract_component_id(item) or None,
    ...
    return _with_org_context(
        {
            "summary": {
                ...
                "filters": {
                    key: value for key, value in payload.items() if key in {"date_from", "date_to", "days"}
                },
            },
            "records": records,
        }
...
def _is_failed_password_change(item: dict[str, Any]) -> bool:
    ...
    failed_tokens = ("fail", "failed", "error", "denied", "失败", "错误")
    reason_tokens = failed_tokens + ("timeout", "invalid", "超时")
    return any(token in status_text for token in failed_tokens) or any(
        token in reason_text for token in reason_tokens
    )
...
def change_password_failure_report(filters: dict[str, Any]) -> dict[str, Any]:
    payload = _normalize_password_change_audit_filters(
        _normalize_time_filters(dict(filters), default_days=7)
    )
    ...
    failed_ids = {id(item) for item in failed_records}
    success_records = [item for item in records if id(item) not in failed_ids]
    ...
    return _with_org_context(
        {
            "summary": {
                ...
                "filters": {
                    ...
                    if key in {"date_from", "date_to", "days", "search", "user", "change_by", "remote_addr", "status"}
                },
            },

...
- 没给组织且语境是报告/使用分析：默认全局组织 `00000000-0000-0000-0000-000000000000`，不写 `.env JMS_ORG_ID`。
- 没给组织且语境是普通查询：使用 `.env JMS_ORG_ID` 当前组织。
- 多组织且没有当前组织：返回 `candidate_orgs`，并通过 `user_message` / `action_hint` 明确要求先选择查询组织。
- 只有 Default 或 Default+内部保留组织可访问时，可自动选择 Default 并写入 `.env JMS_ORG_ID`。
...
报告类请求未显式传组织时走全局组织；普通查询请求未显式传组织时才使用当前组织。
...
- “创建新用户” -> 白名单 create 请求，路由到 create 子 Skill；非白名单写操作才阻塞
- “直接猜一个组织继续查” -> 阻塞

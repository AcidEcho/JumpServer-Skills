...
- 主入口：`python3 subskills/query/scripts/jms_query.py <subcommand> ...`
- 组织对象也走这个入口：`--resource organization`
- 这个入口只提供查询：`object-list`、`object-get` 与 `resolve`
- `object-list`、`object-get`、`resolve` 支持 `--org-id` / `--org-name` 单次命令覆盖组织，不写 `.env`。
...
| 全局用户解析 | 使用 `--org-id 00000000-0000-0000-0000-000000000000` |
...
python3 subskills/query/scripts/jms_query.py resolve --resource user --name openclaw --org-id 00000000-0000-0000-0000-000000000000
```

查询某类资产：

```bash
python3 subskills/query/scripts/jms_query.py object-list --resource asset --kind host --name prod --filter is_active=true
python3 subskills/query/scripts/jms_query.py object-list --resource asset --kind database --filter address=192.0.2.12
```

读取对象详情：

```bash
python3 subskills/query/scripts/jms_query.py object-get --resource asset --id <asset-id>
python3 subskills/query/scripts/jms_query.py object-get --resource platform --id <platform-id>
python3 subskills/query/scripts/jms_query.py object-get --resource user --id <user-id>
```
...
python3 subskills/query/scripts/jms_query.py object-list --resource user-group --name 运维组
```

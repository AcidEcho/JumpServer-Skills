...
| `subskills/access/` | `subskills/access/agents/openai.yaml` | `scripts/jms_ssh_connect.py connect-info` |
...
- Query 的 `subskills/query/scripts/jms_access.py` 是用户有效访问范围的正式入口；Access 的 `subskills/access/scripts/jms_ssh_connect.py` 只提供 `connect-info`。

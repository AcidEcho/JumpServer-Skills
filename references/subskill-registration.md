...
| `subskills/access/` | `subskills/access/agents/openai.yaml` | `jms_ssh_connect.py connect-info`、`jms_db_connect.py db-query`、`jms_connect.py connect-info`、`jms_ssh_exec.py ssh-exec`、`jms_db_exec.py db-exec` |
...
- Query 的 `subskills/query/scripts/jms_access.py` 是用户有效访问范围的正式入口；Access 的 `jms_ssh_connect.py`/`jms_db_connect.py` 保留 SSH 与 MySQL/MariaDB 兼容入口，`jms_connect.py`、`jms_ssh_exec.py`、`jms_db_exec.py` 提供多协议连接和执行入口。
...
python3 subskills/access/scripts/jms_connect.py --help
python3 subskills/access/scripts/jms_ssh_exec.py --help
python3 subskills/access/scripts/jms_db_exec.py --help
